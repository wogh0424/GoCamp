	...
	<div class="pCategory_items">
		<div class="pCategory_search">
			<ul>
				...
				<li><c:if test="${sessionScope.permission == 'ROLE_ADMIN'}">
					<a href="<c:url value="/product/addProduct" />">상품 등록</a>
				</c:if></li>
				<li>
				<form>
		...
		<c:forEach var="dao" items="${list }">
			<div class="product_items">
				<div style="flex: 1;">
					<a href="${cpath }/product/view/${dao.idx}"><img src="${cpath }/resources/shoppingmall/${dao.image}" width="150px" height="150px"></a>
				</div>
				<div style="flex: 3;">
					...
					<c:if test="${sessionScope.permission == 'ROLE_ADMIN'}">
						<a href="<c:url value="/product/modify/${dao.idx}" />">수정</a>
						<a href="<c:url value="/product/delete/${dao.idx}" />">삭제</a>
					</c:if>
					</p>
				</div>
			</div>
